...
function ResourceCrudPage() {
  ...
  const resourceKey = params.resourceKey ?? "";
  const config = resourceConfigMap[resourceKey];
  ...
  if (!config) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Resource Tidak Ditemukan</CardTitle>
          <CardDescription>
            Resource key tidak valid. Pilih menu endpoint dari sidebar.
          </CardDescription>
        </CardHeader>
      </Card>
    );
