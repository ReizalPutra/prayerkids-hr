...
return [
    ...
    'intro_text' => (function (): string {
        $lang = env('SCRIBE_DOC_LANG', 'id');
        $introPath = base_path(".scribe/intro.{$lang}.md");

        if (is_file($introPath)) {
            return file_get_contents($introPath) ?: '';
        }

        return 'Dokumentasi API Prayerkids HR.';
    })(),
    ...
    'auth' => [
        ...
        'extra_info' => (function (): string {
            $lang = env('SCRIBE_DOC_LANG', 'id');

            return $lang === 'en'
                ? 'Use the Bearer token from the login endpoint. Header format: <code>Authorization: Bearer {token}</code>.'
                : 'Gunakan token Bearer dari endpoint login. Format header: <code>Authorization: Bearer {token}</code>.';
        })(),
    ],
